...
class Word {
    ...
    // false if it doesn't
    async isAnswer (gamenum) {
        const g = new Game(gamenum);
    ...
    async isPangram (gamenum) {
        ...
        if (await this.isAnswer(gamenum) === true && intersection.size === 7) {  
            return true;
        } else {
            return false;
        }
    ...
    async calculatePoints (gamenum) {

        // if the word is 4 letters, return 1 point
        if (this.wordLength() === 4 && await this.isAnswer(gamenum)) {
            return 1;
        }
        // if the word is a pangram, return word length + 7
        else if (this.wordLength() > 4 && await this.isPangram(gamenum) && await this.isAnswer(gamenum)) {
            return this.wordLength() + 7;
        }
        // if the word is greater than 4 letters but not a pangram,
        // return word length
        else if (this.wordLength() > 4 && await this.isPangram(gamenum) === false && await this.isAnswer(gamenum)) {
